...
اجعل كل شيء احترافي، جذاب، ومناسب لـ ${formData.platform} والجمهور في ${formData.country}.`;
    ...
    // تتبع التحليلات
    analytics.trackPromptGenerated({
  ...
  const copyToClipboard = () => {
    ...
    analytics.trackPromptCopied(formData.contentType);
  };
  ...
  const downloadPrompt = () => {
    ...
    analytics.trackPromptDownloaded(formData.contentType);
  };

  const shareWhatsApp = () => {
    const siteUrl = typeof window !== 'undefined' ? window.location.origin : '';
    const message = encodeURIComponent(`🎉 لقيت أداة رهيبة لتوليد محتوى السوشيال ميديا لطب الأسنان! جربها مجاناً: ${siteUrl}`);
    window.open(`https://wa.me/?text=${message}`, '_blank');
    
    analytics.trackShare('whatsapp');
  };

  const openFeedback = () => {
    window.open('https://wa.me/201XXXXXXXXX?text=جربت مولد المحتوى الطبي وعايز أقول رأيي 💚', '_blank');
